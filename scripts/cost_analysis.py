...
class CostAnalyzer:
    ...
    def get_cost_and_usage(
        ...
        Returns:
            Dict containing cost and usage data, with all pages of
            ResultsByTime merged (the API returns at most one page per call;
            large accounts and DAILY granularity paginate via NextPageToken)
        """
        if metrics is None:
        ...
        merged: Dict = {"ResultsByTime": []}
        try:
            while True:
                response = self.client.get_cost_and_usage(**params)
                merged["ResultsByTime"].extend(response.get("ResultsByTime", []))
    ...
    def analyze_costs(
        ...
        # Aggregate across time periods so each dimension value appears once
        # for the whole window (a 30-day query spans two calendar months and
        # would otherwise produce duplicate rows per service).
        ...
        for result in response.get("ResultsByTime", []):
            for group in result.get("Groups", []):
                ...
                if dimension_value not in totals:
                    totals[dimension_value] = {
                ...
                totals[dimension_value]["Cost"] += cost
